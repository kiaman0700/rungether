...
`mobile/.env.example`을 복사해 Expo용 Supabase 공개 환경 변수를 설정합니다.
백그라운드 GPS와 오디오 혼합은 Expo Go가 아닌 EAS 개발 빌드에서
검증해야 합니다.

## 구현 범위

- Expo 모바일 앱: 백그라운드 GPS 러닝, 음성 안내, 오프라인 복구, 인증 스튜디오
- 데스크톱 웹: 기록 분석, 크루 운영, SNS 피드 관리
...
- 광역 지역과 시·군·구를 기준으로 하는 크루 생성·검색
- 1:1·그룹·크루 DM과 함께 러닝 모집 데이터 구조
- Supabase RLS, 러닝 완료 RPC, XP·랭킹·크루 기여 데이터 구조
